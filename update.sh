...
commit_if_needed() {
  ...
  git commit -m "$message" >/dev/null 2>&1 || echo "Commit impossible, la mise à jour continue quand même."
}
...
sync_git_before_update() {
  ...
  git pull --rebase origin main >/dev/null 2>&1 || echo "Git pull ignoré : conflit ou authentification GitHub. La mise à jour locale continue."
}
...
push_git_after_everything() {
  ...
  git pull --rebase origin main >/dev/null 2>&1 || echo "Git pull final ignoré : conflit ou authentification GitHub."
...
if ! command -v git >/dev/null 2>&1; then echo "Erreur : Git n'est pas installé ou pas détecté."; pause_exit 1; fi
if ! command -v unzip >/dev/null 2>&1; then echo "Erreur : unzip n'est pas disponible dans Git Bash."; pause_exit 1; fi
...
copy_dir "data"
copy_file "README.md"
copy_file "README-CINEMA-UPDATE.txt"
...
if [ -f "$PROJECT_DIR/backend/package.json" ]; then
  ...
  if command -v node >/dev/null 2>&1 && command -v npm >/dev/null 2>&1; then
    ...
    if [ -f ".env" ] && grep -q "^OMDB_API_KEY=" ".env"; then
      echo "Remplissage du cache synopsis OMDb vers backend/data/imdb-synopsis-cache.json..."
      npm run sync-imdb || echo "sync-imdb échoué, le backend reste utilisable avec fallback TMDB."
    else
      echo "OMDB_API_KEY manquante dans backend/.env : cache synopsis non rempli."
    fi
